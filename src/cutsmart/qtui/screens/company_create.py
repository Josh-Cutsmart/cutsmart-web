...
class CompanyCreateScreen(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QLabel("Set a company name and private join code.")
        ...
        self.name.setStyleSheet(self._entry_css())
        layout.addWidget(self.name)

        self.join_code = QLineEdit()
        self.join_code.setPlaceholderText("Company code / password")
        self.join_code.setEchoMode(QLineEdit.EchoMode.Password)
        self.join_code.setMinimumHeight(46)
        self.join_code.setStyleSheet(self._entry_css())
        self.join_code.returnPressed.connect(self._create)
        layout.addWidget(self.join_code)
    ...
    def _create(self) -> None:
        ...
        try:
            ...
            company_id = self.app.company.create_company(
                uid=uid,
                name=self.name.text().strip(),
                join_code=self.join_code.text(),
            )
            ...
            QMessageBox.information(self, "Company created", f"Company ID: {company_id}")
            self.on_done()
